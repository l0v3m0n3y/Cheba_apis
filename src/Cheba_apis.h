...
class Cheba_apis {
private:
    std::string api_base = "https://extinct-api.herokuapp.com/api/v1";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        // Set headers
        request.headers().add(U("Host"), U("extinct-api.herokuapp.com"));
...
public:
    ...
    pplx::task<json::value> random_animal(bool imageRequired=false) {
        std::map<std::string, std::string> params;
        if (imageRequired) params["imageRequired"] = imageRequired;
        return make_api_call("/animal/" + build_query_params(params),"GET");
    }

    pplx::task<json::value> random_animal(int id,bool imageRequired=false) {
        std::map<std::string, std::string> params;
        if (imageRequired) params["imageRequired"] = imageRequired;
        return make_api_call("/animal/" + std::to_string(id) + build_query_params(params),"GET");
    }
};
